...
def main():
    ...
    # Query counting S/M/L for each TSIC Category, including both Thai and English descriptions
    query = """
    SELECT 
    ...
    # Add totals and calculate shares
    pivot_df['Total'] = pivot_df['S'] + pivot_df['M'] + pivot_df['L']
    pivot_df['S_share'] = pivot_df['S'] / pivot_df['Total']
    pivot_df['M_share'] = pivot_df['M'] / pivot_df['Total']
    pivot_df['L_share'] = pivot_df['L'] / pivot_df['Total']
    ...
    # Standard size for tight layouts
    ...
    # 100% shares
    s_shares = result_df['S_share'].values
    m_shares = result_df['M_share'].values
    l_shares = result_df['L_share'].values
    ...
    ax.invert_yaxis()  # Invert so largest S is on top
    ...
    ax.set_xlabel('สัดส่วนจำนวนธุรกิจ (%) / Share of Firms (%)', fontsize=12, fontweight='bold', labelpad=10)
    
    # Isolate Title and Subtitle to prevent overlaps, while keeping spacing tight
    fig.suptitle('โครงสร้างจำนวนธุรกิจแบ่งตามขนาดธุรกิจและหมวดธุรกิจ TSIC', fontsize=16, fontweight='bold', x=0.5, y=0.96, ha='center')
    ax.set_title('คำนวณจากจำนวนนิติบุคคล เรียงตามหมวดธุรกิจ (A - U)', 
    ...
    chart_path = project_root / "output" / "chart" / "firm_size_share_by_category.png"
