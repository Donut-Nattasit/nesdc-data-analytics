...
def fetch_with_retry(sid, session, max_retries=5):
    ...
    for attempt in range(max_retries):
        ...
        print(f"      [Attempt {attempt+1}/{max_retries}] Empty response or rate limit for {sid}. Waiting {delay:.1f}s before retry...")
        time.sleep(delay)
...
def main():
    print("==========================================================")
    print("[CPI Ingestion] Preparing CPI and Weight Master Dataset...")
    print("==========================================================")
    ...
    # 2. Try to load cached raw data from DB first
    ...
    # 3. Fetch missing series from CEIC
    df_new = pd.DataFrame()
    if missing_sids:
    ...
    if not df_cached.empty or not df_new.empty:
        ...
        # Deduplicate to prevent double-inserting if run multiple times
    ...
    # 4. Ingest Exogenous Dubai Oil Price Prediction
    print("\n[Step 4] Ingesting Exogenous Dubai Crude Oil Forecasts...")
    ...
    # Derive weights:
    # 1. Utilities weight = Energy weight - Motor Fuel weight
    ...
    # 2. ex Utilities weight = Housing & Furnishing weight - Utilities weight
    df_clean["CPI: Weights: Housing & Furnishing (HF): ex Utilities"] = (
        df_clean["CPI: Weights: Housing & Furnishing (HF)"] - 
        df_clean["CPI: Weights: Housing & Furnishing (HF): Utilities"]
    )
    # 3. ex Motor Fuel weight = TC weight - Motor Fuel weight
    ...
    # Derive Indices:
    # 1. Housing & Furnishing: Utilities index = Electricity, Fuel, Water Supply index
    df_clean["CPI: Housing & Furnishing (HF): Utilities"] = df_clean["CPI: HF: Electricity, Fuel, Water Supply"]
    
    # 2. Housing & Furnishing: ex Utilities index = (HF * weight_HF - Utilities * weight_Utilities) / weight_ex_Utilities
    ...
    # 3. Transport & Communication: Motor Fuel index = Motor Fuel Index
    df_clean["CPI: Transport & Communication (TC): Motor Fuel"] = df_clean["CPI: TC: Vehicles & Vehicle Operation: Motor Fuel"]
    
    # 4. Transport & Communication: ex Motor Fuel index = (TC * weight_TC - Motor Fuel * weight_Motor_Fuel) / weight_ex_Motor_Fuel
    ...
    # Ensure all columns exist in df_clean before subsetting
    ...
    out_path = project_root / "output" / "data" / "cpi_forecast" / "cpi_historical_master.csv"
