...
DATA_DIR   = ROOT / "output" / "data"  / "energy_price_forecast_LR"
CHART_DIR  = ROOT / "output" / "chart" / "energy_price_forecast_LR"
MODEL_DIR  = ROOT / "output" / "model_summary" / "energy_price_forecast_LR"
REPORT_DIR = ROOT / "report" / "energy_price_forecast_LR"
...
WATERMARK = "Source: CEIC API & NESDC auto_ARIMA Forecast Model (LR)"
...
# ─────────────────────────────────────────────────────────────────────────────
# CHART 1 — Executive Summary: Annual growth bar chart (LR: 2021–2031)
# ─────────────────────────────────────────────────────────────────────────────
def chart_exec_annual_growth(annual):
    ...
    width = 0.10  # Wider bars for 7 years
    offsets = np.linspace(-0.3, 0.3, len(years))

    hist_color = "#64748b"
    fc_colors  = ["#ffe8cc", "#ffd8a8", "#ffc078", "#ffa94d", "#ff922b", "#fd7e14"]

    handles = []
    for i, yr in enumerate(years):
        off = offsets[i]
        vals = [df_plot.loc[yr, c] for c in composites]
        ...
        for bar in bars:
            ...
            offset_y = 3 if height >= 0 else -3
            ax.annotate(f"{height:+.1f}%" if height != 0 else "0.0%",
                        xy=(bar.get_x() + bar.get_width() / 2, height),
                        xytext=(0, offset_y),
                        textcoords="offset points",
                        ha='center', va=va, fontsize=7, fontweight='semibold',
                        rotation=90)
    ...
    ax.set_title("Thailand CPI Annual Growth Rate — Historical (2025) & Forecast (2026–2031)",
                 fontsize=13, fontweight="bold", pad=14, color="#2c3e50")
    
    # Pad y limits to prevent text clipping
    ymin, ymax = ax.get_ylim()
    ax.set_ylim(ymin * 1.25 if ymin < 0 else ymin, ymax * 1.25 if ymax > 0 else ymax)
...
# ─────────────────────────────────────────────────────────────────────────────
# CHART 5 — Contribution to Growth: Composites
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# CHART 6 — Contribution to Growth: Component
# ─────────────────────────────────────────────────────────────────────────────
...
def chart_forecast_monthly_yoy(monthly):
    ...
    ax.set_title("Thailand CPI — Monthly YoY Growth Forecast (Jan 2026–Dec 2031)",
                 fontsize=13, fontweight="bold", pad=12, color="#2c3e50")
    ax.set_ylabel("YoY Growth (%)", fontsize=10, color="#2c3e50")
    
    # 6-year monthly time scale needs year-based major, 6-month minor ticks
    ax.xaxis.set_major_locator(mdates.YearLocator())
    ax.xaxis.set_major_formatter(mdates.DateFormatter("%Y"))
...
def chart_forecast_quarterly_yoy(quarterly):
    ...
    ax.set_title("CPI Quarterly YoY Growth Forecast (2026–2031)",
                 fontsize=13, fontweight="bold", pad=12, color="#2c3e50")
    ax.set_ylabel("YoY Growth (%)", fontsize=10, color="#2c3e50")
    ax.legend(fontsize=9, frameon=True, facecolor="white", edgecolor="none")
    apply_style(ax)
...
def chart_forecast_annual_yoy(annual):
    ...
    ylim = ax.get_ylim()
    ax.text(5.2, ylim[0] + (ylim[1] - ylim[0]) * 0.88, "FORECAST",
            color="#e74c3c", fontweight="bold", fontsize=9)
    ax.set_title("CPI Annual YoY Growth — History & Forecast (2021–2031)",
                 fontsize=13, fontweight="bold", pad=12, color="#2c3e50")
    ax.set_ylabel("YoY Growth (%)", fontsize=10, color="#2c3e50")
...
# ─────────────────────────────────────────────────────────────────────────────
# CHART 9.5 — Forecast: Annual Contribution Stacked Bar (2021-2031)
# ─────────────────────────────────────────────────────────────────────────────
def chart_forecast_annual_contribution(annual, monthly):
    print("\n[9.5] Forecast annual contribution stacked bar...")
    composites = ["Headline_CPI", "Core_CPI", "Raw_Food_CPI", "Energy_CPI"]
    df_yoy = annual[composites].pct_change() * 100
    df_yoy = df_yoy.loc["2021":"2031"]
    df_yoy.index = df_yoy.index.year
    ...
    core_w = sum(latest.get(c.replace("index_","weight_"), 0) for c in COMPONENT_COLS if COMPONENT_GROUPS[c]=="Core")
    raw_w  = sum(latest.get(c.replace("index_","weight_"), 0) for c in COMPONENT_COLS if COMPONENT_GROUPS[c]=="Raw Food")
    ener_w = sum(latest.get(c.replace("index_","weight_"), 0) for c in COMPONENT_COLS if COMPONENT_GROUPS[c]=="Energy")
    total_w = core_w + raw_w + ener_w

    contrib = pd.DataFrame(index=df_yoy.index)
    contrib["Core"]     = df_yoy["Core_CPI"]     * (core_w / total_w)
    contrib["Raw Food"] = df_yoy["Raw_Food_CPI"] * (raw_w  / total_w)
    contrib["Energy"]   = df_yoy["Energy_CPI"]   * (ener_w / total_w)
    ...
    for grp in ["Core", "Raw Food", "Energy"]:
        ...
        pos = np.where(vals > 0, vals, 0)
        neg = np.where(vals < 0, vals, 0)
        ax.bar(xs, pos, bottom=bottom_pos, color=GROUP_COLORS[grp], label=grp, width=0.55, zorder=3)
        ax.bar(xs, neg, bottom=bottom_neg, color=GROUP_COLORS[grp], width=0.55, zorder=3)
        bottom_pos += pos
        bottom_neg += neg

    # Plot Headline YoY as a line with markers
    ax.plot(xs, df_yoy["Headline_CPI"], color="#2c3e50", linewidth=2.5, marker="o", label="Headline CPI (YoY)", zorder=5)

    # Add labels on the line points with a white background bounding box to prevent overlap
    for x_val, y_val in zip(xs, df_yoy["Headline_CPI"]):
        if pd.isna(y_val):
            continue
        va = 'bottom' if y_val >= 0 else 'top'
        offset_y = 5 if y_val >= 0 else -12
        ax.annotate(f"{y_val:+.1f}%" if y_val != 0 else "0.0%",
                    xy=(x_val, y_val),
                    xytext=(0, offset_y),
                    textcoords="offset points",
                    ha='center', va=va, fontsize=8, fontweight='bold',
                    bbox=dict(boxstyle="round,pad=0.2", fc="white", ec="#2c3e50", lw=0.5, alpha=0.85))

    # Forecast separator at 2025.5
    ax.axvline(2025.5, color="#e74c3c", linestyle="--", linewidth=1.2, alpha=0.8)
    ax.axvspan(2025.5, 2031.5, color="#f5f6fa", alpha=0.9, zorder=0)
    
    ax.axhline(0, color="#7f8c8d", linewidth=0.8)
    ax.set_xticks(xs)
    ax.set_xticklabels(xs.tolist(), fontsize=10)
    ax.set_xlim(2020.4, 2031.6)
    
    ylim = ax.get_ylim()
    # Adjust y limits to make room for annotations
    ax.set_ylim(ylim[0] - 2 if ylim[0] < 0 else -2, ylim[1] + 3)
    ylim = ax.get_ylim()
    
    ax.text(2026.2, ylim[0] + (ylim[1] - ylim[0]) * 0.90, "FORECAST",
            color="#e74c3c", fontweight="bold", fontsize=9, alpha=0.8)
            
    ax.set_title("Contribution to Headline CPI Annual Growth (YoY) by Group",
                 fontsize=13, fontweight="bold", pad=12, color="#2c3e50")
    ax.set_ylabel("Contribution (ppt) / YoY Growth (%)", fontsize=10, color="#2c3e50")
    ax.legend(loc="upper right", fontsize=9, frameon=True, facecolor="white", edgecolor="none")
    apply_style(ax)
    return save(fig, "chart_forecast_annual_contribution.png")
...
def chart_appendix_weight_area_composite(monthly):
    ...
    df = df.replace(0, np.nan).dropna(how="all").ffill()
    df = df.loc["2007":]
...
def chart_appendix_weight_area_component(monthly):
    ...
    df_w = hist[w_cols].loc["2018-01-01":].replace(0, np.nan).dropna(how="all").ffill()
...
# ─────────────────────────────────────────────────────────────────────────────
# CHART 12 — Appendix: Component YoY multi-panel (LR through 2031)
# ─────────────────────────────────────────────────────────────────────────────
def chart_appendix_component_yoy(monthly):
    ...
    fc_start = monthly[monthly["is_forecast"] == 1].index.min()
    ...
    for i, col in enumerate(COMPONENT_COLS):
        ...
        # Dynamic x-axis tick spacing to handle full historical horizon without crowding
        years_range = (ser.index.max() - ser.index.min()).days / 365.25
        if years_range > 30:
            interval = 10
        elif years_range > 15:
            interval = 5
    ...
    fig.suptitle("CPI Component YoY Growth (%) — Monthly (Historical & Long-Range Forecast)",
                 fontsize=13, fontweight="bold", y=0.96, color="#2c3e50")
    # Removed fig.autofmt_xdate(rotation=30) to preserve individual x-axis labels on all subplots
    return save(fig, "chart_appendix_component_yoy.png", rect=[0, 0.03, 1, 0.92])

# ─────────────────────────────────────────────────────────────────────────────
# CHART 13 — Appendix: Component Index Level multi-panel (LR through 2031)
# ─────────────────────────────────────────────────────────────────────────────
def chart_appendix_component_level(monthly):
    print("\n[13] Appendix component index level 4x4 multi-panel...")
    df_all = monthly[COMPONENT_COLS + ["is_forecast"]].copy()
    ...
    fig.suptitle("CPI Component Index Level — Monthly (Historical & Forecast, dashed)",
                 fontsize=13, fontweight="bold", y=0.96, color="#2c3e50")
    # Removed fig.autofmt_xdate(rotation=30) to preserve individual x-axis labels on all subplots
...
def build_monthly_yoy_table(monthly):
    ...
    df_yoy = df_yoy.loc["2026-01":"2031-12"].dropna(subset=composites, how="all")
...
def build_quarterly_yoy_table(quarterly):
    composites = ["Headline_CPI", "Core_CPI", "Raw_Food_CPI", "Energy_CPI"]
    df_yoy = quarterly[composites].pct_change(4) * 100
    df_yoy = df_yoy.loc["2026":"2031"].dropna(how="all").round(2)
...
def build_annual_yoy_table(annual):
    composites = ["Headline_CPI", "Core_CPI", "Raw_Food_CPI", "Energy_CPI"]
    df_yoy = annual[composites].pct_change() * 100
    df_yoy = df_yoy.loc["2021":"2031"].dropna(how="all").round(2)
    df_yoy.index = df_yoy.index.year
    df_yoy.columns = ["Headline (%)", "Core (%)", "Raw Food (%)", "Energy (%)"]
    return df_yoy
...
def build_component_tables(monthly, quarterly, annual):
    ...
    m  = _slice_and_label(monthly_yoy,   "2026-01", "2031-12", "%b %Y")
    q  = _slice_and_label(quarterly_yoy, "2026",    "2031",    "%b %Y")
    a  = _slice_and_label(annual_yoy,    "2021",    "2031",    "%Y")
    a.index = [int(d) for d in a.index]
    return m, q, a

def build_oil_price_tables(monthly, annual):
    df_m = monthly.loc["2026-06-01":"2031-12-01", ["exog_dubai_crude"]].copy()
    df_m.index = df_m.index.strftime("%b %Y")
    df_m.columns = ["Dubai Crude Price (USD/bbl)"]
    df_m = df_m.round(2)

    df_a = annual.loc["2021-01-01":"2031-12-01", ["exog_dubai_crude"]].copy()
    df_a.index = df_a.index.year
    df_a.columns = ["Dubai Crude Price (USD/bbl)"]
    df_a = df_a.round(2)
    return df_m, df_a
...
def compile_report(monthly, quarterly, annual, chart_paths):
    print("\n[Compiling Markdown report (LR)...]")
    report_path = REPORT_DIR / "energy_price_forecast_LR.md"
    ...
    oil_m_tbl, oil_a_tbl = build_oil_price_tables(monthly, annual)
    ...
    hl_2031 = float(ann_yoy.loc[ann_yoy.index.year == 2031, "Headline_CPI"].iloc[0])

    def rel(png_name):
        return f"../../output/chart/energy_price_forecast_LR/{png_name}"

    md = f"""# Thailand CPI Forecasting Report (Long-Range Scenario)
...
The NESDC auto-ARIMA models, using the custom long-range Dubai spot price forecast as an exogenous variable, project Headline CPI growth to **recover to {hl_2026:.2f}%** in 2026, before stabilizing at **{hl_2031:.2f}%** by 2031. Core inflation is projected to stay extremely stable under this baseline, hovering around the 0.9–1.0% range.
...
The pie chart below shows the share of each composite group in the total CPI basket, based on the most recent available weights (as of **{latest_wt_date.strftime('%B %Y')}**).
...
*Source: CEIC API & NESDC Calculation, {latest_wt_date.strftime('%Y')}.*
...
![Contribution to Growth — Annual Forecast]({rel('chart_forecast_annual_contribution.png')})
...
def main():
    print("=" * 60)
    print("  CPI Forecast Report Generator (LR Scenario)")
    print("=" * 60)
    ...
    chart_paths["forecast_annual_contribution"] = chart_forecast_annual_contribution(annual, monthly)
    ...
    for key, path in chart_paths.items():
        try:
            ...
            add_visualization(
                name=f"CPI Forecast (LR) — {key.replace('_', ' ').title()}",
                chart_type="Mixed",
                source_data="output/data/energy_price_forecast_LR/",
                png_path=rel_path,
                status="Rendered"
            )
    ...
    try:
        ...
        add_report(
            title="Thailand CPI Forecasting Report (Long-Range Scenario)",
