...
│   │   ├── raw/           # Raw, unedited API fetch outputs
...
* **`World_Trend_Plus.db`** (stored in `output/data/database/`): Local repository for global trend indicators and historical time series.
* **`GTA.db`** (stored in `output/data/database/`): Contains Global Trade Atlas records, import/export series, and transaction details.

For detailed schema details, index guides, and sample queries optimized for SQL-generating AI assistants, refer to **[output/data/database/README.md](file:///C:/Users/natta/OneDrive%20-%20nesdc.go.th/NESDC/Gemini/data-analysis/output/data/database/README.md)**.
